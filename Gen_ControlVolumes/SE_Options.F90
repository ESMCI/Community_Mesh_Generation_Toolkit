...
!============================================================================
  ...
  real(kind=real_kind):: hypervis_scaling     = 0    ! use tensor hyperviscosity
  real(kind=real_kind):: fine_ne              = 0
  real(kind=real_kind):: max_hypervis_courant = 1d99 ! upper bound for Courant number
...
contains
  ...
  subroutine Init_SEoptions(O_SEopt,I_NameListFile)
    ...
    logical                      :: create_SCRIP_file  = .false.
    logical                      :: create_LATLON_file = .false.
    ...
    if(present(I_NameListFile)) then
      ...
      call find_group_name(NLunit,'ctl_nl',status=ierr)
      if(ierr.eq.0) then
        read(NLunit,ctl_nl,iostat=ierr)
        if(ierr.ne.0) then
          call endrun("ERROR Reading ctl_nl namelist values")
        endif
      else
        call endrun("ERROR Finding ctl_nl namelist in file")
      endif
      close(NLunit)
    endif
