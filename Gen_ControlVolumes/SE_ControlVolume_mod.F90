...
contains
  ...
  subroutine Calc_ControlVolumes(elem,I_SEopt,O_SEcv)
    ...
    O_SEcv%subelement_corners(:,:)         = subelement_corners(:,:)
    O_SEcv%area              (:)           = area2d            (:)
    O_SEcv%lonp              (:)           = lonp              (:)
    O_SEcv%latp              (:)           = latp              (:)
  ...
  subroutine Create_SCRIP_file(I_SEcv,I_FileName)
    ...
    real(kind=real_kind),allocatable:: grid_imask     (:)
    ...
    print *,' '
    retval = nf90_create(trim(I_FileName),ior(NF90_NOCLOBBER,NF90_NETCDF4),ncid)
    if(retval.ne.0) then
      print *,' retval=',retval
    ...
    retval = nf90_def_var(ncid,'grid_imask'     ,NF90_DOUBLE,(/I_grid_size/),I_grid_imask     )
