...
!                 components needed.
!
!      Init_SEelem_Transformations(SEelem)  :  PRIVATE routine
!                 Given the SEelem values initialize from Init_BASEelem(), compute
!                 all of the additional tranformation values needed for accuate 
...
!===================================================================================
  ...
  private:: Init_SEelem_Transformations
  public :: Set_Element_Surfaces
  public :: SE_Get_Ncol
  public :: SE_Get_Nelem
  public :: SE_Get_Face
  ...
  type SEelem_t
    ...
    real(kind=real_kind):: gradx_Z(np,np,nlev) ! gradient of model surface height
    real(kind=real_kind):: grady_Z(np,np,nlev) ! gradient of model surface height
    real(kind=real_kind):: d1zm   (np,np,nlev) ! 1st derivative coef in vertical
    real(kind=real_kind):: d1z0   (np,np,nlev) ! 1st derivative coef in vertical
    real(kind=real_kind):: d1zp   (np,np,nlev) ! 1st derivative coef in vertical
    real(kind=real_kind):: d2zm   (np,np,nlev) ! 2nd derivative coef in vertical
    real(kind=real_kind):: d2z0   (np,np,nlev) ! 2nd derivative coef in vertical
    real(kind=real_kind):: d2zp   (np,np,nlev) ! 2nd derivative coef in vertical
    ! Augmented values
    !------------------
    real(kind=real_kind):: A(np,np)
    real(kind=real_kind):: B(np,np)
    real(kind=real_kind):: ALPHA(np,np)
    real(kind=real_kind):: BETA (np,np)
    real(kind=real_kind):: RHO  (np,np)
    real(kind=real_kind):: Bcov_Sphere2Ref(2,2,np,np)
    real(kind=real_kind):: Bcov_Ref2Sphere(2,2,np,np)
    real(kind=real_kind):: Vcon_Sphere2Ref(2,2,np,np)
    real(kind=real_kind):: Vcon_Ref2Sphere(2,2,np,np)
    real(kind=real_kind):: Vcov_Sphere2Ref(2,2,np,np)
    real(kind=real_kind):: Vcov_Ref2Sphere(2,2,np,np)
    real(kind=real_kind):: Tmix_Sphere2Ref(2,2,2,2,np,np)
    real(kind=real_kind):: Tmix_Ref2Sphere(2,2,2,2,np,np)
    real(kind=real_kind):: IJKref(2,2,2,np,np)
    real(kind=real_kind):: Gcon(2,2,np,np)
    real(kind=real_kind):: Gcov(2,2,np,np)
    real(kind=real_kind):: Gdet(np,np)
    real(kind=real_kind):: tanLat(np,np)
    real(kind=real_kind):: cosLat(np,np)
  end type SEelem_t
...
contains
  ...
  subroutine Init_SEelem(IO_SEopt,O_SEelem,O_SEcv)
    ...
    real(kind=real_kind):: LatVal,LatMin,LatMax,LatDist
    integer:: ie,ii,jj
...
!XX      endif
      ...
      ! Loop over element points, compute tan(Lat), cos(Lat) values 
      ! and deal with the poles when they pop up.
      !-------------------------------------------------------------
!DIAG      LatDist = 1.e-3
      LatDist = 5.e-3
      LatMax  =  (DD_PI/2.d0)-LatDist
      LatMin  = -(DD_PI/2.d0)+LatDist
      do jj=1,np
      do ii=1,np
        LatVal = O_SEelem(ie)%spherep(ii,jj)%lat
        if(LatVal.gt.LatMax) then
          LatVal = LatMax
        elseif(LatVal.lt.LatMin) then
          LatVal = LatMin
        endif
        O_SEelem(ie)%tanLat(ii,jj) = tan(LatVal)
        O_SEelem(ie)%cosLat(ii,jj) = cos(LatVal)
      end do
      end do
    end do

    ! Now compute additional values needed to implement accurate
    ! differential operators and related computations.
    !-----------------------------------------------------------
    do ie=1,IO_SEopt%nelem
      call Init_SEelem_Transformations(O_SEelem(ie))
    end do
  ...
  subroutine Init_BASEelem(IO_SEopt,elem)
    ...
    if(IO_SEopt%ne.eq.0) then
      ...
      IO_SEopt%max_elements_attached_to_node = 7                  ! variable resolution
...
!     print*, 'Using max_unif_dx = ', max_unif_dx
  ...
  !==================================================================
  subroutine Init_SEelem_Transformations(SEelem)
    !
    ! Init_SEelem_Transformations: For the given element, set all of the additional 
    !                              tranformation values needed for accuate derivative 
    !                              computations.
    !=================================================================================
    !
    ! Passed Variables
    !------------------
    type(SEelem_t),intent(inout):: SEelem
    !
    ! Local Values
    !--------------
    type(quadrature_t)  :: gp
    real(kind=real_kind):: a(np),b(np),Ap(np),Am(np),Bp(np),Bm(np)
    real(kind=real_kind):: ALPH1,ALPH2,ALPH3,ALPH4
    real(kind=real_kind):: BETA1,BETA2,BETA3,BETA4
    real(kind=real_kind):: Rval,Scoef,Ccoef,Acoef,Bcoef,Dcoef
    real(kind=real_kind):: d2ALPHdAB,d2BETAdAB
    real(kind=real_kind)::    tanA(np,np),   tanB(np,np),   Dval(np,np)
    real(kind=real_kind)::   tanAB(np,np),  tanAA(np,np),  tanBB(np,np)
    real(kind=real_kind):: dALPHda(np,np),dALPHdb(np,np),dBETAda(np,np),dBETAdb(np,np)
    real(kind=real_kind):: dLAMBda(np,np),dLAMBdb(np,np),dTHETda(np,np),dTHETdb(np,np)
    real(kind=real_kind):: dadALPH(np,np),dadBETA(np,np),dbdALPH(np,np),dbdBETA(np,np)
    real(kind=real_kind):: dadLAMB(np,np),dadTHET(np,np),dbdLAMB(np,np),dbdTHET(np,np)
    real(kind=real_kind):: IJKang(2,2,2,np,np)
    integer:: ii,jj,nn
    logical:: diag

    diag = .false.

    ! Normalized?: Earth Radius or unit sphere?   --- verify
    !-------------------------------------------
    Rval = 1.d0

    ! Initialize reference gridpoint values [-1,+1]
    !-------------------------------------------------
    gp    = gausslobatto(np)
    do nn=1,np
      a (nn) = gp%points(nn)
      b (nn) = gp%points(nn)
      Ap(nn) = (1.d0+a(nn))/2.d0
      Am(nn) = (1.d0-a(nn))/2.d0
      Bp(nn) = (1.d0+b(nn))/2.d0
      Bm(nn) = (1.d0-b(nn))/2.d0
    end do

    ! Get Angular values at element corners
    !-------------------------------------------------
    ALPH1 = SEelem%corners(1)%x
    ALPH2 = SEelem%corners(4)%x
    ALPH3 = SEelem%corners(3)%x
    ALPH4 = SEelem%corners(2)%x
    BETA1 = SEelem%corners(1)%y
    BETA2 = SEelem%corners(4)%y
    BETA3 = SEelem%corners(3)%y
    BETA4 = SEelem%corners(2)%y

    ! Initialize reference angular values [-PI/4,+PI/4]
    !-------------------------------------------------
    do jj=1,np
    do ii=1,np
      SEelem%A(ii,jj) = a(ii)
      ...
      SEelem%ALPHA(ii,jj) = Am(ii)*Bm(jj)*ALPH1 + Am(ii)*Bp(jj)*ALPH2 &
                          + Ap(ii)*Bp(jj)*ALPH3 + Ap(ii)*Bm(jj)*ALPH4
      SEelem%BETA (ii,jj) = Am(ii)*Bm(jj)*BETA1 + Am(ii)*Bp(jj)*BETA2 &
                          + Ap(ii)*Bp(jj)*BETA3 + Ap(ii)*Bm(jj)*BETA4

      tanA (ii,jj)      = tan(SEelem%ALPHA(ii,jj))
      tanB (ii,jj)      = tan(SEelem%BETA (ii,jj))
      tanAB(ii,jj)      = tanA(ii,jj)*tanB(ii,jj)
      tanAA(ii,jj)      = 1.d0 + tanA(ii,jj)**2
      tanBB(ii,jj)      = 1.d0 + tanB(ii,jj)**2
      SEelem%RHO(ii,jj) = sqrt(1.d0 + tanA(ii,jj)**2 + tanB(ii,jj)**2)
    end do
    end do

    ! Initailize differentials
    !--------------------------
    d2ALPHdAB = (ALPH1 - ALPH2 + ALPH3 - ALPH4)/4.d0
    d2BETAdAB = (BETA1 - BETA2 + BETA3 - BETA4)/4.d0

    do jj=1,np
    do ii=1,np
      ! Compute [a,b] <--> [ALPHA,BETA] differentails
      !-----------------------------------------------
      dALPHda(ii,jj) = Bm(jj)*(ALPH4-ALPH1)/2.d0 + Bp(jj)*(ALPH3-ALPH2)/2.d0
      dALPHdb(ii,jj) = Am(ii)*(ALPH2-ALPH1)/2.d0 + Ap(ii)*(ALPH3-ALPH4)/2.d0
      dBETAda(ii,jj) = Bm(jj)*(BETA4-BETA1)/2.d0 + Bp(jj)*(BETA3-BETA2)/2.d0
      dBETAdb(ii,jj) = Am(ii)*(BETA2-BETA1)/2.d0 + Ap(ii)*(BETA3-BETA4)/2.d0

      Dval   (ii,jj) = dALPHda(ii,jj)*dBETAdb(ii,jj) - dALPHdb(ii,jj)*dBETAda(ii,jj)
      dadALPH(ii,jj) =  dBETAdb(ii,jj)/Dval(ii,jj)
      dadBETA(ii,jj) = -dALPHdb(ii,jj)/Dval(ii,jj)
      dbdALPH(ii,jj) = -dBETAda(ii,jj)/Dval(ii,jj)
      dbdBETA(ii,jj) =  dALPHda(ii,jj)/Dval(ii,jj)

      ! Compute [a,b] <--> [LAMBDA,THETA] differentails
      !-----------------------------------------------
      Scoef = Rval/((SEelem%RHO(ii,jj)**2)*cos(SEelem%ALPHA(ii,jj)))
      Ccoef = SEelem%RHO(ii,jj)*cos(SEelem%ALPHA(ii,jj))                       &
                             *((cos(SEelem%BETA (ii,jj))**2))/(Rval*Dval(ii,jj))

      dLAMBda(ii,jj) = Scoef*SEelem%RHO(ii,jj)*dALPHda(ii,jj)
      dLAMBdb(ii,jj) = Scoef*SEelem%RHO(ii,jj)*dALPHdb(ii,jj)
      dTHETda(ii,jj) = Scoef*(   -tanAB(ii,jj)*dALPHda(ii,jj) + tanBB(ii,jj)*dBETAda(ii,jj))
      dTHETdb(ii,jj) = Scoef*(   -tanAB(ii,jj)*dALPHdb(ii,jj) + tanBB(ii,jj)*dBETAdb(ii,jj))
      dadLAMB(ii,jj) = Ccoef*(   -tanAB(ii,jj)*dALPHdb(ii,jj) + tanBB(ii,jj)*dBETAdb(ii,jj))
      dbdLAMB(ii,jj) = Ccoef*(    tanAB(ii,jj)*dALPHda(ii,jj) - tanBB(ii,jj)*dBETAda(ii,jj))
      dadTHET(ii,jj) = Ccoef*SEelem%RHO(ii,jj)*dALPHdb(ii,jj)
      dbdTHET(ii,jj) = Ccoef*SEelem%RHO(ii,jj)*dALPHda(ii,jj)
    end do
    end do

    !======================================================
    ! Optionally use the current SEelem D(), Dinv() values
    !======================================================
!PFC-diag+++
    if(diag) then
      write(iulog,*) 'PFC:---------------------------'
      write(iulog,*) 'PFC: SEelem%A(ii,jj) =',SEelem%A(:,:)
      write(iulog,*) 'PFC: SEelem%B(ii,jj) =',SEelem%B(:,:)
      write(iulog,*) 'PFC: ALPHA=',SEelem%ALPHA(:,:)
      write(iulog,*) 'PFC: BETA =',SEelem%BETA (:,:)
      write(iulog,*) 'PFC: RHO  =',SEelem%RHO  (:,:)
      write(iulog,*) 'PFC: d2ALPHdAB =',d2ALPHdAB
      write(iulog,*) 'PFC: d2BETAdAB =',d2BETAdAB
      write(iulog,*) 'PFC: Dval =',Dval(:,:)
      write(iulog,*) 'PFC: dALPHda =',dALPHda(:,:)
      write(iulog,*) 'PFC: dALPHdb =',dALPHdb(:,:)
      write(iulog,*) 'PFC: dBETAda =',dBETAda(:,:)
      write(iulog,*) 'PFC: dBETAdb =',dBETAdb(:,:)
      write(iulog,*) 'PFC: dadALPH =',dadALPH(:,:)
      write(iulog,*) 'PFC: dadBETA =',dadBETA(:,:)
      write(iulog,*) 'PFC: dbdALPH =',dbdALPH(:,:)
      write(iulog,*) 'PFC: dbdBETA =',dbdBETA(:,:)
      write(iulog,*) 'PFC: dLAMBda =',dLAMBda(:,:)
      write(iulog,*) 'PFC: dLAMBdb =',dLAMBdb(:,:)
      write(iulog,*) 'PFC: dTHETda =',dTHETda(:,:)
      write(iulog,*) 'PFC: dTHETdb =',dTHETdb(:,:)
      write(iulog,*) 'PFC: dadLAMB =',dadLAMB(:,:)
      write(iulog,*) 'PFC: dbdLAMB =',dbdLAMB(:,:)
      write(iulog,*) 'PFC: dadTHET =',dadTHET(:,:)
      write(iulog,*) 'PFC: dbdTHET =',dbdTHET(:,:)
      write(iulog,*) 'PFC: LocalId = SEelem%LocalId =',SEelem%LocalId
      write(iulog,*) 'PFC: GlobalId= SEelem%GlobalId=',SEelem%GlobalId
      write(iulog,*) 'PFC: dLAMBda = SEelem%D(1,1,:,:) =',SEelem%D(1,1,:,:)
      write(iulog,*) 'PFC: dLAMBdb = SEelem%D(1,2,:,:) =',SEelem%D(1,2,:,:)
      write(iulog,*) 'PFC: dTHETda = SEelem%D(2,1,:,:) =',SEelem%D(2,1,:,:)
      write(iulog,*) 'PFC: dTHETdb = SEelem%D(2,2,:,:) =',SEelem%D(2,2,:,:)
      write(iulog,*) 'PFC: dadLAMB = SEelem%Dinv(1,1,:,:) =',SEelem%Dinv(1,1,:,:)
      write(iulog,*) 'PFC: dadTHET = SEelem%Dinv(1,2,:,:) =',SEelem%Dinv(1,2,:,:)
      write(iulog,*) 'PFC: dbdLAMB = SEelem%Dinv(2,1,:,:) =',SEelem%Dinv(2,1,:,:)
      write(iulog,*) 'PFC: dbdTHET = SEelem%Dinv(2,2,:,:) =',SEelem%Dinv(2,2,:,:)
      write(iulog,*) 'PFC:---------------------------'
      write(iulog,*) 'PFC:'
    endif
!PFC-diag===
   ...
   IF(.TRUE.) THEN
    do jj=1,np
    do ii=1,np
      dLAMBda(ii,jj) = SEelem%D(1,1,ii,jj)
      dLAMBdb(ii,jj) = SEelem%D(1,2,ii,jj)
      dTHETda(ii,jj) = SEelem%D(2,1,ii,jj)
      dTHETdb(ii,jj) = SEelem%D(2,2,ii,jj)
      dadLAMB(ii,jj) = SEelem%Dinv(1,1,ii,jj)
      dadTHET(ii,jj) = SEelem%Dinv(1,2,ii,jj)
      dbdLAMB(ii,jj) = SEelem%Dinv(2,1,ii,jj)
      dbdTHET(ii,jj) = SEelem%Dinv(2,2,ii,jj)
    end do
    end do
   ENDIF

    ! Initialize Basis transformation Arrays
    !----------------------------------------
    do jj=1,np
    do ii=1,np
      SEelem%Bcov_Sphere2Ref(1,1,ii,jj) = dLAMBda(ii,jj)
      SEelem%Bcov_Sphere2Ref(1,2,ii,jj) = dTHETda(ii,jj)
      SEelem%Bcov_Sphere2Ref(2,1,ii,jj) = dLAMBdb(ii,jj)
      SEelem%Bcov_Sphere2Ref(2,2,ii,jj) = dTHETdb(ii,jj)

      SEelem%Bcov_Ref2Sphere(1,1,ii,jj) = dadLAMB(ii,jj)
      SEelem%Bcov_Ref2Sphere(1,2,ii,jj) = dbdLAMB(ii,jj)
      SEelem%Bcov_Ref2Sphere(2,1,ii,jj) = dadTHET(ii,jj)
      SEelem%Bcov_Ref2Sphere(2,2,ii,jj) = dbdTHET(ii,jj)
    end do
    end do
    ...
    do jj=1,np
    do ii=1,np
      SEelem%Gcov(1,1,ii,jj) = dLAMBda(ii,jj)*dLAMBda(ii,jj) + dTHETda(ii,jj)*dTHETda(ii,jj)
      SEelem%Gcov(1,2,ii,jj) = dLAMBda(ii,jj)*dLAMBdb(ii,jj) + dTHETda(ii,jj)*dTHETdb(ii,jj)
      SEelem%Gcov(2,1,ii,jj) = SEelem%Gcov(1,2,ii,jj)
      SEelem%Gcov(2,2,ii,jj) = dLAMBdb(ii,jj)*dLAMBdb(ii,jj) + dTHETdb(ii,jj)*dTHETdb(ii,jj)
!DIAG      SEelem%Gdet(    ii,jj) = dLAMBda(ii,jj)*dTHETdb(ii,jj) - dTHETda(ii,jj)*dLAMBdb(ii,jj)
!DIAG      SEelem%Gcov(1,1,ii,jj) = dadLAMB(ii,jj)*dadLAMB(ii,jj) + dadTHET(ii,jj)*dadTHET(ii,jj)
!DIAG      SEelem%Gcov(1,2,ii,jj) = dadLAMB(ii,jj)*dbdLAMB(ii,jj) + dadTHET(ii,jj)*dbdTHET(ii,jj)
!DIAG      SEelem%Gcov(2,1,ii,jj) = SEelem%Gcov(1,2,ii,jj)
!DIAG      SEelem%Gcov(2,2,ii,jj) = dbdLAMB(ii,jj)*dbdLAMB(ii,jj) + dbdTHET(ii,jj)*dbdTHET(ii,jj)
      SEelem%Gdet(    ii,jj) = SEelem%Gcov(1,1,ii,jj)*SEelem%Gcov(2,2,ii,jj) &
                              -SEelem%Gcov(2,1,ii,jj)*SEelem%Gcov(1,2,ii,jj)
      SEelem%Gcon(1,1,ii,jj) =  SEelem%Gcov(2,2,ii,jj)/SEelem%Gdet(ii,jj)
      SEelem%Gcon(1,2,ii,jj) = -SEelem%Gcov(1,2,ii,jj)/SEelem%Gdet(ii,jj)
      SEelem%Gcon(2,1,ii,jj) = -SEelem%Gcov(2,1,ii,jj)/SEelem%Gdet(ii,jj)
      SEelem%Gcon(2,2,ii,jj) =  SEelem%Gcov(1,1,ii,jj)/SEelem%Gdet(ii,jj)
    end do
    end do

    ! Initialize Vector transformation Arrays
    !----------------------------------------
    do jj=1,np
    do ii=1,np
      SEelem%Vcon_Sphere2Ref(1,1,ii,jj) = dadLAMB(ii,jj)
      SEelem%Vcon_Sphere2Ref(1,2,ii,jj) = dadTHET(ii,jj)
      SEelem%Vcon_Sphere2Ref(2,1,ii,jj) = dbdLAMB(ii,jj)
      SEelem%Vcon_Sphere2Ref(2,2,ii,jj) = dbdTHET(ii,jj)

      SEelem%Vcon_Ref2Sphere(1,1,ii,jj) = dLAMBda(ii,jj)
      SEelem%Vcon_Ref2Sphere(1,2,ii,jj) = dLAMBdb(ii,jj)
      SEelem%Vcon_Ref2Sphere(2,1,ii,jj) = dTHETda(ii,jj)
      SEelem%Vcon_Ref2Sphere(2,2,ii,jj) = dTHETdb(ii,jj)

      SEelem%Vcov_Sphere2Ref(1,1,ii,jj) = dLAMBda(ii,jj)
      SEelem%Vcov_Sphere2Ref(1,2,ii,jj) = dTHETda(ii,jj)
      SEelem%Vcov_Sphere2Ref(2,1,ii,jj) = dLAMBdb(ii,jj)
      SEelem%Vcov_Sphere2Ref(2,2,ii,jj) = dTHETdb(ii,jj)

      SEelem%Vcov_Ref2Sphere(1,1,ii,jj) = dadLAMB(ii,jj)
      SEelem%Vcov_Ref2Sphere(1,2,ii,jj) = dbdLAMB(ii,jj)
      SEelem%Vcov_Ref2Sphere(2,1,ii,jj) = dadTHET(ii,jj)
      SEelem%Vcov_Ref2Sphere(2,2,ii,jj) = dbdTHET(ii,jj)
    end do
    end do

    ! Initialize 2nd order mixed tensor transformation Arrays
    !--------------------------------------------------------
 IF(.TRUE.) THEN
    do jj=1,np
    do ii=1,np
      SEelem%Tmix_Sphere2Ref(1,1,1,1,ii,jj) = dadLAMB(ii,jj)*dLAMBda(ii,jj)
      SEelem%Tmix_Sphere2Ref(1,2,1,1,ii,jj) = dadLAMB(ii,jj)*dLAMBdb(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,1,1,1,ii,jj) = dbdLAMB(ii,jj)*dLAMBda(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,2,1,1,ii,jj) = dbdLAMB(ii,jj)*dLAMBdb(ii,jj)

      SEelem%Tmix_Sphere2Ref(1,1,1,2,ii,jj) = dadLAMB(ii,jj)*dTHETda(ii,jj)
      SEelem%Tmix_Sphere2Ref(1,2,1,2,ii,jj) = dadLAMB(ii,jj)*dTHETdb(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,1,1,2,ii,jj) = dbdLAMB(ii,jj)*dTHETda(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,2,1,2,ii,jj) = dbdLAMB(ii,jj)*dTHETdb(ii,jj)

      SEelem%Tmix_Sphere2Ref(1,1,2,1,ii,jj) = dadTHET(ii,jj)*dLAMBda(ii,jj)
      SEelem%Tmix_Sphere2Ref(1,2,2,1,ii,jj) = dadTHET(ii,jj)*dLAMBdb(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,1,2,1,ii,jj) = dbdTHET(ii,jj)*dLAMBda(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,2,2,1,ii,jj) = dbdTHET(ii,jj)*dLAMBdb(ii,jj)

      SEelem%Tmix_Sphere2Ref(1,1,2,2,ii,jj) = dadTHET(ii,jj)*dTHETda(ii,jj)
      SEelem%Tmix_Sphere2Ref(1,2,2,2,ii,jj) = dadTHET(ii,jj)*dTHETdb(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,1,2,2,ii,jj) = dbdTHET(ii,jj)*dTHETda(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,2,2,2,ii,jj) = dbdTHET(ii,jj)*dTHETdb(ii,jj)

      SEelem%Tmix_Ref2Sphere(1,1,1,1,ii,jj) = dadLAMB(ii,jj)*dLAMBda(ii,jj)
      SEelem%Tmix_Ref2Sphere(1,2,1,1,ii,jj) = dadTHET(ii,jj)*dLAMBda(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,1,1,1,ii,jj) = dadLAMB(ii,jj)*dTHETda(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,2,1,1,ii,jj) = dadTHET(ii,jj)*dTHETda(ii,jj)

      SEelem%Tmix_Ref2Sphere(1,1,1,2,ii,jj) = dbdLAMB(ii,jj)*dLAMBda(ii,jj)
      SEelem%Tmix_Ref2Sphere(1,2,1,2,ii,jj) = dbdTHET(ii,jj)*dLAMBda(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,1,1,2,ii,jj) = dbdLAMB(ii,jj)*dTHETda(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,2,1,2,ii,jj) = dbdTHET(ii,jj)*dTHETda(ii,jj)

      SEelem%Tmix_Ref2Sphere(1,1,2,1,ii,jj) = dadLAMB(ii,jj)*dLAMBdb(ii,jj)
      SEelem%Tmix_Ref2Sphere(1,2,2,1,ii,jj) = dadTHET(ii,jj)*dLAMBdb(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,1,2,1,ii,jj) = dadLAMB(ii,jj)*dTHETdb(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,2,2,1,ii,jj) = dadTHET(ii,jj)*dTHETdb(ii,jj)

      SEelem%Tmix_Ref2Sphere(1,1,2,2,ii,jj) = dbdLAMB(ii,jj)*dLAMBdb(ii,jj)
      SEelem%Tmix_Ref2Sphere(1,2,2,2,ii,jj) = dbdTHET(ii,jj)*dLAMBdb(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,1,2,2,ii,jj) = dbdLAMB(ii,jj)*dTHETdb(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,2,2,2,ii,jj) = dbdTHET(ii,jj)*dTHETdb(ii,jj)
    end do
    end do
 ELSE
   ! Tranposed ?
    do jj=1,np
    do ii=1,np
      SEelem%Tmix_Sphere2Ref(1,1,1,1,ii,jj) = dLAMBda(ii,jj)*dadLAMB(ii,jj)
      SEelem%Tmix_Sphere2Ref(1,2,1,1,ii,jj) = dLAMBda(ii,jj)*dbdLAMB(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,1,1,1,ii,jj) = dLAMBdb(ii,jj)*dadLAMB(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,2,1,1,ii,jj) = dLAMBdb(ii,jj)*dbdLAMB(ii,jj)

      SEelem%Tmix_Sphere2Ref(1,1,1,2,ii,jj) = dLAMBda(ii,jj)*dadTHET(ii,jj)
      SEelem%Tmix_Sphere2Ref(1,2,1,2,ii,jj) = dLAMBda(ii,jj)*dbdTHET(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,1,1,2,ii,jj) = dLAMBdb(ii,jj)*dadTHET(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,2,1,2,ii,jj) = dLAMBdb(ii,jj)*dbdTHET(ii,jj)

      SEelem%Tmix_Sphere2Ref(1,1,2,1,ii,jj) = dTHETda(ii,jj)*dadLAMB(ii,jj)
      SEelem%Tmix_Sphere2Ref(1,2,2,1,ii,jj) = dTHETda(ii,jj)*dbdLAMB(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,1,2,1,ii,jj) = dTHETdb(ii,jj)*dadLAMB(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,2,2,1,ii,jj) = dTHETdb(ii,jj)*dbdLAMB(ii,jj)

      SEelem%Tmix_Sphere2Ref(1,1,2,2,ii,jj) = dTHETda(ii,jj)*dadTHET(ii,jj)
      SEelem%Tmix_Sphere2Ref(1,2,2,2,ii,jj) = dTHETda(ii,jj)*dbdTHET(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,1,2,2,ii,jj) = dTHETdb(ii,jj)*dadTHET(ii,jj)
      SEelem%Tmix_Sphere2Ref(2,2,2,2,ii,jj) = dTHETdb(ii,jj)*dbdTHET(ii,jj)

      SEelem%Tmix_Ref2Sphere(1,1,1,1,ii,jj) = dLAMBda(ii,jj)*dadLAMB(ii,jj)
      SEelem%Tmix_Ref2Sphere(1,2,1,1,ii,jj) = dTHETda(ii,jj)*dadLAMB(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,1,1,1,ii,jj) = dLAMBda(ii,jj)*dadTHET(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,2,1,1,ii,jj) = dTHETda(ii,jj)*dadTHET(ii,jj)

      SEelem%Tmix_Ref2Sphere(1,1,1,2,ii,jj) = dLAMBdb(ii,jj)*dadLAMB(ii,jj)
      SEelem%Tmix_Ref2Sphere(1,2,1,2,ii,jj) = dTHETdb(ii,jj)*dadLAMB(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,1,1,2,ii,jj) = dLAMBdb(ii,jj)*dadTHET(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,2,1,2,ii,jj) = dTHETdb(ii,jj)*dadTHET(ii,jj)

      SEelem%Tmix_Ref2Sphere(1,1,2,1,ii,jj) = dLAMBda(ii,jj)*dbdLAMB(ii,jj)
      SEelem%Tmix_Ref2Sphere(1,2,2,1,ii,jj) = dTHETda(ii,jj)*dbdLAMB(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,1,2,1,ii,jj) = dLAMBda(ii,jj)*dbdTHET(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,2,2,1,ii,jj) = dTHETda(ii,jj)*dbdTHET(ii,jj)

      SEelem%Tmix_Ref2Sphere(1,1,2,2,ii,jj) = dLAMBdb(ii,jj)*dbdLAMB(ii,jj)
      SEelem%Tmix_Ref2Sphere(1,2,2,2,ii,jj) = dTHETdb(ii,jj)*dbdLAMB(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,1,2,2,ii,jj) = dLAMBdb(ii,jj)*dbdTHET(ii,jj)
      SEelem%Tmix_Ref2Sphere(2,2,2,2,ii,jj) = dTHETdb(ii,jj)*dbdTHET(ii,jj)
    end do
    end do
 ENDIF

    ! Initialize equ-angular connection coefs (Christoffell Symbols) 
    !---------------------------------------------------------------
    do jj=1,np
    do ii=1,np
      Bcoef = tanB(ii,jj)/(SEelem%RHO(ii,jj)**2)
      IJKang(1,1,1,ii,jj) = Bcoef*( 2.d0*tanA(ii,jj)*tanB(ii,jj))
      IJKang(1,1,2,ii,jj) = Bcoef*(-tanBB(ii,jj))
      IJKang(1,2,1,ii,jj) = Bcoef*(-tanBB(ii,jj))
      IJKang(1,2,2,ii,jj) = 0.d0

      Acoef = tanA(ii,jj)/(SEelem%RHO(ii,jj)**2)
      IJKang(2,1,1,ii,jj) = 0.d0
      IJKang(2,1,2,ii,jj) = Acoef*(-tanAA(ii,jj))
      IJKang(2,2,1,ii,jj) = Acoef*(-tanAA(ii,jj))
      IJKang(2,2,2,ii,jj) = Acoef*( 2.d0*tanA(ii,jj)*tanB(ii,jj))
    end do
    end do
...
!PFC-diag+++
    if(diag) then
      write(iulog,*) 'PFC:---------------------------'
      write(iulog,*) 'PFC: IJKang(1,1,1)=',IJKang(1,1,1,:,:)
      write(iulog,*) 'PFC: IJKang(1,1,2)=',IJKang(1,1,2,:,:)
      write(iulog,*) 'PFC: IJKang(1,2,1)=',IJKang(1,2,1,:,:)
      write(iulog,*) 'PFC: IJKang(1,2,2)=',IJKang(1,2,2,:,:)
      write(iulog,*) 'PFC: IJKang(2,1,1)=',IJKang(2,1,1,:,:)
      write(iulog,*) 'PFC: IJKang(2,1,2)=',IJKang(2,1,2,:,:)
      write(iulog,*) 'PFC: IJKang(2,2,1)=',IJKang(2,2,1,:,:)
      write(iulog,*) 'PFC: IJKang(2,2,2)=',IJKang(2,2,2,:,:)
      write(iulog,*) 'PFC:---------------------------'
      write(iulog,*) 'PFC:'
    endif
!PFC-diag===

    ! Transform equ-angular coefs to refernce element
    !-------------------------------------------------
    do jj=1,np
    do ii=1,np
      Acoef = dALPHdb(ii,jj)/Dval(ii,jj)
      Bcoef = dBETAdb(ii,jj)/Dval(ii,jj)
      Dcoef = 1.d0/Dval(ii,jj)
      SEelem%IJKref(1,1,1,ii,jj) = Bcoef*(IJKang(1,1,1,ii,jj)*dALPHda(ii,jj)*dALPHda(ii,jj)  &
                                         +IJKang(1,1,2,ii,jj)*dALPHda(ii,jj)*dBETAda(ii,jj)  &
                                         +IJKang(1,2,1,ii,jj)*dBETAda(ii,jj)*dALPHda(ii,jj)) &
                                  -Acoef*(IJKang(2,2,2,ii,jj)*dBETAda(ii,jj)*dBETAda(ii,jj)  &
                                         +IJKang(2,1,2,ii,jj)*dALPHda(ii,jj)*dBETAda(ii,jj)  &
                                         +IJKang(2,2,1,ii,jj)*dBETAda(ii,jj)*dALPHda(ii,jj))

      SEelem%IJKref(1,1,2,ii,jj) = Bcoef*(IJKang(1,1,1,ii,jj)*dALPHda(ii,jj)*dALPHdb(ii,jj)  &
                                         +IJKang(1,1,2,ii,jj)*dALPHda(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(1,2,1,ii,jj)*dBETAda(ii,jj)*dALPHdb(ii,jj)) &
                                  -Acoef*(IJKang(2,2,2,ii,jj)*dBETAda(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(2,1,2,ii,jj)*dALPHda(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(2,2,1,ii,jj)*dBETAda(ii,jj)*dALPHdb(ii,jj)) &
                                  +Dcoef*(dBETAdb(ii,jj)*d2ALPHdAB - dALPHdb(ii,jj)*d2BETAdAB)

      SEelem%IJKref(1,2,1,ii,jj) = SEelem%IJKref(1,1,2,ii,jj)

      SEelem%IJKref(1,2,2,ii,jj) = Bcoef*(IJKang(1,1,1,ii,jj)*dALPHdb(ii,jj)*dALPHdb(ii,jj)  &
                                         +IJKang(1,1,2,ii,jj)*dALPHdb(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(1,2,1,ii,jj)*dBETAdb(ii,jj)*dALPHdb(ii,jj)) &
                                  -Acoef*(IJKang(2,2,2,ii,jj)*dBETAdb(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(2,1,2,ii,jj)*dALPHdb(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(2,2,1,ii,jj)*dBETAdb(ii,jj)*dALPHdb(ii,jj))

      Acoef = dALPHda(ii,jj)/Dval(ii,jj)
      Bcoef = dBETAda(ii,jj)/Dval(ii,jj)
      Dcoef = 1.d0/Dval(ii,jj)
      SEelem%IJKref(2,1,1,ii,jj) = Acoef*(IJKang(2,2,2,ii,jj)*dBETAda(ii,jj)*dBETAda(ii,jj)  &
                                         +IJKang(2,1,2,ii,jj)*dALPHda(ii,jj)*dBETAda(ii,jj)  &
                                         +IJKang(2,2,1,ii,jj)*dBETAda(ii,jj)*dALPHda(ii,jj)) &
                                  -Bcoef*(IJKang(1,1,1,ii,jj)*dALPHda(ii,jj)*dALPHda(ii,jj)  &
                                         +IJKang(1,1,2,ii,jj)*dALPHda(ii,jj)*dBETAda(ii,jj)  &
                                         +IJKang(1,2,1,ii,jj)*dBETAda(ii,jj)*dALPHda(ii,jj))

      SEelem%IJKref(2,1,2,ii,jj) = Acoef*(IJKang(2,2,2,ii,jj)*dBETAda(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(2,1,2,ii,jj)*dALPHda(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(2,2,1,ii,jj)*dBETAda(ii,jj)*dALPHdb(ii,jj)) &
                                  -Bcoef*(IJKang(1,1,1,ii,jj)*dALPHda(ii,jj)*dALPHdb(ii,jj)  &
                                         +IJKang(1,1,2,ii,jj)*dALPHda(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(1,2,1,ii,jj)*dBETAda(ii,jj)*dALPHdb(ii,jj)) &
                                  +Dcoef*(dALPHda(ii,jj)*d2BETAdAB - dBETAda(ii,jj)*d2ALPHdAB)

      SEelem%IJKref(2,2,1,ii,jj) = SEelem%IJKref(2,1,2,ii,jj)

      SEelem%IJKref(2,2,2,ii,jj) = Acoef*(IJKang(2,2,2,ii,jj)*dBETAdb(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(2,1,2,ii,jj)*dALPHdb(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(2,2,1,ii,jj)*dBETAdb(ii,jj)*dALPHdb(ii,jj)) &
                                  -Bcoef*(IJKang(1,1,1,ii,jj)*dALPHdb(ii,jj)*dALPHdb(ii,jj)  &
                                         +IJKang(1,1,2,ii,jj)*dALPHdb(ii,jj)*dBETAdb(ii,jj)  &
                                         +IJKang(1,2,1,ii,jj)*dBETAdb(ii,jj)*dALPHdb(ii,jj))
    end do
    end do
...
!PFC-diag+++
    if(diag) then
      write(iulog,*) 'PFC:---------------------------'
      write(iulog,*) 'PFC: IJKref(1,1,1)=',SEelem%IJKref(1,1,1,:,:)
      write(iulog,*) 'PFC: IJKref(1,1,2)=',SEelem%IJKref(1,1,2,:,:)
      write(iulog,*) 'PFC: IJKref(1,2,1)=',SEelem%IJKref(1,2,1,:,:)
      write(iulog,*) 'PFC: IJKref(1,2,2)=',SEelem%IJKref(1,2,2,:,:)
      write(iulog,*) 'PFC: IJKref(2,1,1)=',SEelem%IJKref(2,1,1,:,:)
      write(iulog,*) 'PFC: IJKref(2,1,2)=',SEelem%IJKref(2,1,2,:,:)
      write(iulog,*) 'PFC: IJKref(2,2,1)=',SEelem%IJKref(2,2,1,:,:)
      write(iulog,*) 'PFC: IJKref(2,2,2)=',SEelem%IJKref(2,2,2,:,:)
      write(iulog,*) 'PFC:---------------------------'
      write(iulog,*) 'PFC:'
    endif
!PFC-diag===

    ! Initialize all of the surface values to zero. These should be set to 
    ! the correct values using Set_Element_Surfaces() prior to use.
    !----------------------------------------------------------------------
    SEelem%gradx_Z(:,:,:) = 0.d0
    SEelem%grady_Z(:,:,:) = 0.d0
    SEelem%d1zm   (:,:,:) = 0.d0
    ...
    !--------------
    return
  end subroutine Init_SEelem_Transformations
  !==================================================================

  !==================================================================
  subroutine Set_Element_Surfaces(I_Zval,I_Gradx_Z,I_Grady_Z,SEelem)
    !
    ! Set_Element_Surfaces: Use the given Surface height values (in meters) and
    !                       corresponding gradient values to set the surface
    !                       information for the elements. 
    !=================================================================================
    !
    ! Passed Variables
    !------------------
    real(kind=real_kind),intent(in   ):: I_Zval   (SEnp,SEnp,SEnlev,SEnelem)
    real(kind=real_kind),intent(in   ):: I_Gradx_Z(SEnp,SEnp,SEnlev,SEnelem)
    real(kind=real_kind),intent(in   ):: I_Grady_Z(SEnp,SEnp,SEnlev,SEnelem)
    type(SEelem_t)      ,intent(inout):: SEelem(:)
    !
    ! Local Values
    !--------------
    real(kind=real_kind):: Det(SEnp,SEnp)
    integer:: ie,kk,km,k0,kp

    ! Just copy the gradient values to the element structure for later use
    !----------------------------------------------------------------------
    do ie=1,SEnelem
      SEelem(ie)%gradx_Z(:,:,:) = I_Gradx_Z(:,:,:,ie)
      SEelem(ie)%grady_Z(:,:,:) = I_Grady_Z(:,:,:,ie)
    end do

    ! Use the Z heights to compute coefs for vertical derivative estimates
    !----------------------------------------------------------------------
    do ie=1,SEnelem
      ! k=1
      !------
      ...
      k0 = 2
      kp = 3
      Det(:,:) = (I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie))*(I_Zval(:,:,kp,ie)**2) &
                -(I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie))*(I_Zval(:,:,k0,ie)**2) &
                +(I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))*(I_Zval(:,:,km,ie)**2)
      SEelem(ie)%d1zm(:,:,km) = (2.d0*I_Zval(:,:,km,ie)-I_Zval(:,:,k0,ie)-I_Zval(:,:,kp,ie)) &
                               *(I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))/Det(:,:)
      SEelem(ie)%d1z0(:,:,km) =  ((I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie))**2)/Det(:,:)
      SEelem(ie)%d1zp(:,:,km) = -((I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie))**2)/Det(:,:)
      SEelem(ie)%d2zm(:,:,km) =  2.d0*(I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))/Det(:,:)
      SEelem(ie)%d2z0(:,:,km) = -2.d0*(I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie))/Det(:,:)
      SEelem(ie)%d2zp(:,:,km) =  2.d0*(I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie))/Det(:,:)
      ...
      Det(:,:) = (I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie))*(I_Zval(:,:,kp,ie)**2) &
                -(I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie))*(I_Zval(:,:,k0,ie)**2) &
                +(I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))*(I_Zval(:,:,km,ie)**2)
      SEelem(ie)%d1zm(:,:,kp) =  ((I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))**2)/Det(:,:)
      SEelem(ie)%d1z0(:,:,kp) = -((I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie))**2)/Det(:,:)
      SEelem(ie)%d1zp(:,:,kp) = (2.d0*I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie)) &
                                *(I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie))/Det(:,:)
      SEelem(ie)%d2zm(:,:,kp) =  2.d0*(I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))/Det(:,:)
      SEelem(ie)%d2z0(:,:,kp) = -2.d0*(I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie))/Det(:,:)
      SEelem(ie)%d2zp(:,:,kp) =  2.d0*(I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie))/Det(:,:)
...
!!                  -(I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie))*(I_Zval(:,:,k0,ie)**2) &
!!                  +(I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))*(I_Zval(:,:,km,ie)**2)
!!        SEelem(ie)%d1zm(:,:,k0) = -((I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))**2)/Det(:,:)
!!        SEelem(ie)%d1z0(:,:,k0) = (I_Zval(:,:,kp,ie)-2.d0*I_Zval(:,:,k0,ie)+I_Zval(:,:,km,ie)) &
!!                                  *(I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie))/Det(:,:)
!!        SEelem(ie)%d1zp(:,:,k0) = -((I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie))**2)/Det(:,:)
!!        SEelem(ie)%d2zm(:,:,k0) =  2.d0*(I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))/Det(:,:)
!!        SEelem(ie)%d2z0(:,:,k0) = -2.d0*(I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie))/Det(:,:)
!!        SEelem(ie)%d2zp(:,:,k0) =  2.d0*(I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie))/Det(:,:)

        Det(:,:) = 2.d0*(I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie)) &
                       *(I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))
        SEelem(ie)%d1zm(:,:,k0) = -(I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie))/Det(:,:)
        SEelem(ie)%d1z0(:,:,k0) = -(2.d0*I_Zval(:,:,k0,ie)-I_Zval(:,:,kp,ie)         &
                                                          -I_Zval(:,:,km,ie))/Det(:,:)
        SEelem(ie)%d1zp(:,:,k0) =  (I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie))/Det(:,:)
        SEelem(ie)%d2zm(:,:,k0) =  2.d0/((I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie)) &
                                        *(I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie)))
        SEelem(ie)%d2z0(:,:,k0) = -2.d0/((I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie)) &
                                        *(I_Zval(:,:,k0,ie)-I_Zval(:,:,km,ie)))
        SEelem(ie)%d2zp(:,:,k0) =  2.d0/((I_Zval(:,:,kp,ie)-I_Zval(:,:,km,ie)) &
                                        *(I_Zval(:,:,kp,ie)-I_Zval(:,:,k0,ie)))
      end do
    end do ! ie=1,SEnelem
    ...
    !--------------
    return
  end subroutine Set_Element_Surfaces
  !==================================================================
